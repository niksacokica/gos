gos:AddApp({
	["name"] = "File Explorer",
	["icon"] = "gos/app_icons/file_explorer.png",
	["creator"] = "niksacokica",
	["window"] = function( window )
		...
		browser.Refresh = function( self )
			...
			self:Clear()
			self:SetPath( "DATA" )
			self:SetBaseFolder( "gos/personal_files" )
		...
		local function warningPopUp( text )
			local warning = gos:CreatePopUp( window, "File explorer warning" )
		...
		local function addPopUp( curPath, title, rnm, folder )
			local popUp = gos:CreatePopUp( window, "File explorer PopUp" )
				...
				for _, v in ipairs( files ) do
					...
					icon.DoRightClick = function( self )
						...
						menu:AddOption( "Open", function()
							local text_editor = gos:StartApp( gos.apps["text editor"] )
		...
		local function delPopUp( curPath, title, folder )
			local popUp = gos:CreatePopUp( window, "File explorer delete" )
			popUp:SetPos( ScrW() * 0.42, ScrH() * 0.42 )
			popUp:SetSize( 300, 150 )
			popUp:SetTitle( title )
			popUp:MakePopup()
