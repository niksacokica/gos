gos:AddApp({
	["name"] = "Text editor",
	["icon"] = "gos/app_icons/text_editor.png",
	["creator"] = "niksacokica",
	["window"] = function( window )
		...
		local function warningPopUp( parent, text )
			local warning = gos:CreatePopUp( parent, "File editor warning" )
		...
		local function selectFile( saveType )
			local fileBrowser = gos:CreatePopUp( window, "Text editor file selector" )
			...
			fileSelector:SetBaseFolder( "gos/personal_files" )
			...
			local function open_save()
				...
				local function alreadyExists( text )
					local warning = gos:CreatePopUp( fileBrowser, "File editor exists warning" )
					warning:SetPos( ScrW() * 0.42, ScrH() * 0.42 )
					warning:SetSize( 300, 150 )
					warning:SetTitle( "WARNING" )
					warning:MakePopup()
			...
			fileSelector.ShowFolder = function(self, path)
				...
				for _, filter in ipairs( string.Explode( " ", filters ) ) do
					...
					for _, v in ipairs( files ) do
						...
						icon:SetMaterial( "gos/files/doc_" .. string.GetExtensionFromFilename( v ) .. ".png" )
						icon:SetName( v )
		...
		local function saveChangesPopUp( saveFunction, noSaveFunction )
			local popUp = gos:CreatePopUp( window, "Text editor save changes" )
		...
		local function Replace()		
			local findReplace = gos:CreatePopUp( window, "Text editor find replace" )
		...
		shortcuts = {
			...
			["newEditor"] = function()
					gos:StartApp( gos.apps["text editor"] )
				end,
			["newFile"] = function()
